...
public abstract class AbstractRender {
     ...
     * @param x            在点(x, y)上缩放
     */
    public void zoom(int visibleCount, int left, int right, int width, int x) {
        if (x < left) {
            x = left;
        }
        if (x > right) {
        ...
        final int toMinVisibleIndex = (visibleCount * (x - left) / width);
        ...
        touchValues[Matrix.MSCALE_X] = EntryManager.getInstance().getEntryList().size() / visibleCount;

        computeScrollRange(width, touchValues[Matrix.MSCALE_X]);

        touchValues[Matrix.MTRANS_X] = getTransX(visibleCount, minVisibleIndex);

        matrixTouch.setValues(touchValues);

        Log.e(TAG, "##d zoom: touchValues[Matrix.MSCALE_X] = " + touchValues[Matrix.MSCALE_X]
    ...
    public void mapPoints(float[] pts, RectF mRectF) {
        matrixValue.mapPoints(pts);
        matrixTouch.mapPoints(pts);
        matrixOffset.mapPoints(pts);
    }
    ...
    protected void postMatrixValue(float width, float height, float minY, float deltaY) {
        final float scaleX = width / EntryManager.getInstance().getEntryList().size();
    ...
    protected void postMatrixTouch(float width, float visibleCount) {
        final float scaleX = EntryManager.getInstance().getEntryList().size() / visibleCount;
    ...
    protected float getTransX(float visibleCount, int entryIndex) {
        final int entrySetSize = EntryManager.getInstance().getEntryList().size();
    ...
    public abstract void onSizeChanged(int left, int top, int right, int bottom);
